const data = JSON.parse(`[{
  ...
  export function getCar(req, res) {
    const { car } = req.params; // turi b8t toks pat kaip get /:car
    const carName = data.filter((c) => c.car === car);

    if (carName.length > 0) {  
    res.json(carName);
  } else {
    res.status(404).json({ message: `Car model: ${car} not found`});
  }
  }

  export function getEmailByID(req, res) {
    const { id } = req.params;
    const userID = data.find((e) => e.id === +id);
...
// rast vis1 sar161 email, .map
  export function getEmailList(req, res) {
    const emailList = data.map((e) => e.email); 

    if (emailList.length > 0) {
        res.json(emailList);
    } else {
        res.status(404).json({ message: `Emails list not found`});
      }
  }

  export function getGender(req, res) {
    const femailList = data.filter((p) => p.gender === "Female") .map((p) => `${p.first_name} ${p.last_name}`);

    if (femailList.length > 0) {
        res.json(femailList);
    } else {
        res.status(404).json({ message: `femail list not found`});
    }
  }
